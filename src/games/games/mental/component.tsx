...
export default class extends Component<any, any> implements Game {
  ...
  public prepareConfig = (result: any) => {
    ...
    return {
      ...
      subtract: result.subtract == 1,
      restriction: result.restriction == 1,
      length,
  ...
  render() {
    const { started = false, list = [] } = this.state;

    const { width, delay = 1000 } = this.props;
    ...
    return (
      ...
      >
        {started && (
          <StartTimer
            time={3}
            renderComponent={() => <WrapperGame delay={delay} onEnd={this.end} list={list} />}
          />
        )}
      </View>
    );
