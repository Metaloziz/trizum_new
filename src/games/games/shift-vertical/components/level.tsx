...
export default class extends Component<any, any> {
  ...
  onSubmit =
    (result = 'error') =>
    () => {
      const { stage, color, form, valiants } = this.state;

      const { onEnd = () => {}, onResult = () => {} } = this.props;
      ...
      if (stage == 1) {
        // Compare color
        if (result == 'success' && valiants.color.color == color) {
          onSuccess();
        } else if (result == 'error' && valiants.color.color !== color) {
          onSuccess();
        } else {
          onError();
        }
      ...
      if (stage == 2) {
        // Compare form
        if (result == 'success' && valiants.form.form == form) {
          onSuccess();
        } else if (result == 'error' && valiants.form.form !== form) {
          onSuccess();
        } else {
          onError();
        }
  ...
  render() {
    const { delay = 3 } = this.props;

    const { stage = 0, color, valiants, form } = this.state;
    ...
    if (stage == 0) {
      return (
        <View style={styles.wrap}>
          ...
          <View>
            <TimerButton onPress={this.onColor} delay={delay} />
          </View>
        </View>
      );
    }
    ...
    return (
      <View style={styles.wrap}>
        ...
        <View style={styles.wrapButtons}>
          ...
          >
            <Text style={styles.buttonTitle}>Не совпадает</Text>
          </TouchableOpacity>
          ...
          >
            <Text style={styles.buttonTitle}>Совпадает</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
